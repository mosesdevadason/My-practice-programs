...
int register_timeout (long int sec, long int usec)
{
    ...
    /* If usec is greater than 100000 add 1 to sec and subtract 1000000 from usec */
    ...
    printf("%s:%s:%d: Inserted new timeout. sec: %ld, usec: %ld\n", 
           __FILE__, __func__, __LINE__, timeout->time.sec, timeout->time.usec);
...
void run_timer (void)
{
    ...
    while(NULL != timeout_head)
    {
        ...
        {
            /* If current time is greater than the first timeout (head) set sec and usec 
             * as 0 */
            tv.sec = tv.usec = 0;
        }
        _tv.tv_sec = tv.sec;
        ...
        {
            /* If return value is -1 and errno is valid and not EINTR throw error log 
             * EINTR means a signal has been caught. So it is not an error case */
            perror("select\n");
            return;
        }
        ...
        if(!os_time_before(&now, &(timeout_head->time)))
        {
            ...
            printf("%s:%s:%d: Timeout occured. sec: %ld, usec: %ld\n",
                   __FILE__, __func__, __LINE__, tmp->time.sec, tmp->time.usec);
