    ...
    *p = nullptr; *n = nullptr;
    do {
        if (0 == index) {
            ...
            *n = (list_node_t *) ((uintptr_t)node ^ (uintptr_t)*p);
            return node;
        }
...
void Xor_linked_list::list_delete(size_t index)
{
    ...
    /* pp is the node previous to the p. This can be obtained by XORing
     * address of current node with xor ptr of p. */
    pp = (list_node_t *) ((uintptr_t)p->xor_ptr ^ (uintptr_t)node);
    /* nn is the node next to n. This can be obtaned by XORing the address of
     * current node and xor_ptr of n. */
    nn = (list_node_t *) ((uintptr_t)node ^ (uintptr_t)n->xor_ptr);
    // Delete node and re-calculate xor_ptrs of p and n.
    delete node;
